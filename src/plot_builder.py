...
def build_plot(audio_file_path):
    criteria = peaks_detector.GetPeaksCriteria(peaks_distance=15, peaks_prominence=0.5, peaks_height=[0, 1], hop_length=512)

    response = peaks_detector.get_peaks_with_sample_rate_with_normalized_energy_with_amplitude_values(audio_file_path, criteria)

    plot_energy_with_peaks(
        response.norm_energy,
        response.peaks,
